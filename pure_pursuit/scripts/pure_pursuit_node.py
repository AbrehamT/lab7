...
class PurePursuit(Node):
    def __init__(self):
        ...
        # self.declare_parameter('wheelbase', 0.39)              # [m]
        # self.declare_parameter('max_steering_angle', 0.4189) # [rad]
# 
        # self.declare_parameter('min_lookahead', 0.5)         # L_min [m]
        # self.declare_parameter('lookahead_gain', 1.25)        # k_v [m per (m/s)]

        # ======================= LOGS_SLOW ONLY =======================

        # vehicle & control parameters
        self.declare_parameter('wheelbase', 0.43)              # [m]
        self.declare_parameter('max_steering_angle', 0.4189) # [rad]
 
        self.declare_parameter('min_lookahead', 0.5)         # L_min [m]
        self.declare_parameter('lookahead_gain', 1.0)        # k_v [m per (m/s)]
        ...
        files = sorted(glob.glob(f"{home}/rcws/logs_slow/raceline-*.csv"))
 

        # ======================= LOGS_FAST ONLY =======================

        #files = sorted(glob.glob(f"{home}/rcws/logs_fast/raceline-*.csv"))
    ...
    def run_pursuit(self):
        ...
        self.Ld_curr += self.alpha * (Ld - self.Ld_curr) * dt
        Ld = self.Ld_curr
        # 3) log for debug
